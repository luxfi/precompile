...
func TestModule_RegisteredAtTheAdvertisedAddress(t *testing.T) {
	...
	require.True(t, ok, "vrf is not registered at %s", ContractAddress)
	require.Equal(t, ConfigKey, m.ConfigKey)
	require.Equal(t, VRFPrecompile, m.Contract)
...
// TestModule_RegisteringTwiceIsRejected pins the guard that stops a second
// module claiming this address or config key.
...
func (otherConfig) Key() string { return "not-vrf" }
