...
// AssetResolver into the 0x9999 cEVM value path. The resolver (which PROVES an asset's
// canonical identity on the node's bound network — permissionless, not an allowlist) is
// backed by chains/dexvm/registry as a metadata CACHE + X-chain reality source; the value
...
// constructs the resolver and installs it here exactly like it installs the local D-Chain
// client (InstallDChainClient).
//
