...
// always-on value path FAILS CLOSED (ErrNoAssetResolver) rather than admitting a
// left-padded address as a real asset. This is the intrinsic safety net: a node that
// serves 0x9999 without wiring the registry refuses every swap (registry admission is part
// of the swap, not a separate activation step).
