...
// the real-asset registry installed, AND native-value swaps ENABLED (the value gate flipped
// on, as a quorum-finality node would). The 0x9999 precompile is always dispatchable, but a
// synchronous fill requires value swaps enabled — so the e2e value-path tests turn the gate
// on (and restore it on cleanup) before routing through the synchronous on-chain router.
...
func newE2EHarness(t *testing.T) *e2eHarness {
	...
	// The 0x9999 value path is ALWAYS-ON (no value-activation gate) — the installed
	// resolver IS the control that gates whether a swap fills.
...
// Test9999PlainSwapIsSynchronous is the one-path proof, ON A VALUE-ENABLED NODE: with native-
// value swaps enabled (newE2EHarness flips the A2 gate on, as a quorum-finality node would), a
// plain swap() against a resting maker SETTLES SYNCHRONOUSLY in-process — the taker's real
// balances move, the maker's book updates, the V4 BalanceDelta is returned in the same call —
// and it creates NO async intent record. The complement (gate OFF -> no synchronous fill) is
// Test9999SyncSwap_ValueGateFailsClosed. (A value-enabled swap() can never mint a Deadline=0
// locked intent — the gate routes it straight to the synchronous router, no async fork.)
