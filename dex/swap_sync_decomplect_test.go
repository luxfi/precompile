...
// gate), and the INSTALLED ASSET RESOLVER is the single control that decides whether a swap
// fills. It pins both halves of the live wiring in one place so Red can re-review the trust
// root:
//
//	(a) a REAL registered asset swaps successfully END-TO-END (resolver installed -> swap
//	    fills, real C-Chain balances move, the D book records the fill);
//	(b) the SAME market with an UNREGISTERED (synthetic) asset FAILS CLOSED at admission
//	    (ErrAssetNotAdmitted), and a node with NO resolver fails closed (ErrNoAssetResolver) —
//	    proving the resolver, not a gate, is what stands between fail-closed and live.
//
// There is NO EnableValueSwaps / ValueSwapsEnabled / quorum gate anywhere in this path: a
// plain swap() reaches the synchronous router and the swap's intrinsic controls (resolver
// admission + live-code verifier + min-out floor + custody mutex) gate the fill.

// TestDecomplect_RealAssetFillsThenSyntheticFailsClosed proves the decomplect end-to-end:
// the unconditional synchronous router fills a swap over a real, registered, code-backed
// market; and on a market whose base is unregistered, the SAME router refuses the swap at
// the admission gate without moving any value.
func TestDecomplect_RealAssetFillsThenSyntheticFailsClosed(t *testing.T) {
	// ── (a) REAL ASSET, RESOLVER INSTALLED → SWAP FILLS ──────────────────────────────────
	// newE2EHarness installs the registry-shaped resolver admitting the market's two real
	// ERC-20s (LETH, LUSD) + native, and gives them live on-chain code — exactly the trust
	// root a value-live node carries (the registry-backed resolver installDEXValuePath wires).
	// There is no value gate to flip: the resolver IS the activation.
	h := newE2EHarness(t)
	...
	// ── (b) UNREGISTERED (SYNTHETIC) ASSET → SAME ROUTER FAILS CLOSED ─────────────────────
	// A fresh harness on a market whose BASE is a fabricated, never-registered asset. The
	// resolver admits only the real quote + native, so the base has no admission. The SAME
	// unconditional synchronous router must REVERT at the market-open (ErrAssetNotAdmitted)
	// and bind NO market — even though the left-pad of the fabricated address is a perfectly
	// well-formed 32-byte assetID.
	hs := newSettleHarness(t)
	hs.key = c1MarketKey() // base = c1FabricatedBase (unregistered), quote = c1RealQuote (real)

	r := newTestAssetResolver(hs.networkID, hs.cChainID)
	r.admitNative(t, 18)
	r.admitERC20(t, c1RealQuote, 6) // ONLY the real quote is registered; the base is not
	hs.state.stateDB.SetCodeSize(c1RealQuote, 1)
	...
	if !errors.Is(serr, dexcore.ErrAssetNotAdmitted) {
		t.Fatalf("a swap over an UNREGISTERED base must fail closed (ErrAssetNotAdmitted), got: %v", serr)
	}
...
// TestDecomplect_InitializeRefusesSyntheticAsset proves the C1 PARITY gate on the initialize
// path (settle_market.go admitMarketAssets): a market RECORD can be created only over two
// registered, enabled, code-backed assets. An initialize over a fabricated base is REFUSED
// (ErrAssetNotAdmitted) and writes NO MarketRecord — closing the phantom-stub surface where
...
func TestDecomplect_InitializeRefusesSyntheticAsset(t *testing.T) {
	...
	h.key = c1MarketKey() // base = c1FabricatedBase (unregistered)

	// Resolver admits only the real quote + native; the base is unregistered. Initialize must
	// run the SAME admission the swap path runs.
	r := newTestAssetResolver(h.networkID, h.cChainID)
	r.admitNative(t, 18)
	r.admitERC20(t, c1RealQuote, 6)
	h.state.stateDB.SetCodeSize(c1RealQuote, 1)
	prev := installedAssetResolver.Load()
	...
	if !errors.Is(err, dexcore.ErrAssetNotAdmitted) {
		t.Fatalf("initialize over an unregistered base must be REFUSED (ErrAssetNotAdmitted), got: %v", err)
	}
...
// serves 0x9999 without wiring the registry cannot even create a market record.
