...
// SEPARATELY from the depositor pot (settleVault). The two are distinct CLAIMS on the
// same vault account and must not raid each other:
//
...
//
//	GetBalance(0x9999)/ERC20 holding of a == settleVault[a] + custody[a]
//
// TWO TERMS, NOT THREE. A maker's committed value is not a third claim: commitPosition
// funds D through Export, which debits the caller and credits custody[a], so it is
// already inside custody. lockedReserve[owner][a] is a per-owner VIEW of that same
// value for bookkeeping, never a separate holding — adding it to the invariant would
// double-count it, and any balance-difference read that subtracts it refuses honest
// deposits. deliveredNative (native_zap.go) is the one reader of this identity.
//
// An import draws ONLY on custody[a], so it can never pay out of a depositor's claim;
// a withdraw draws ONLY on settleVault[a], so it can never strand a backed import.
//
