...
func TestZzmpMoveNativeOutOfVaultAbortsRatherThanWrapping(t *testing.T) {
	...
	if err := moveNativeOutOfVault(db, to, 1); !errors.Is(err, ErrCustodyUnbacked) {
		t.Fatalf("empty vault release: want ErrCustodyUnbacked, got %v", err)
	}
	if got := db.GetBalance(poolManagerAddr9999); got.Sign() != 0 {
	...
	if err := moveNativeOutOfVault(db, to, 51); !errors.Is(err, ErrCustodyUnbacked) {
		t.Fatalf("one-over release: want ErrCustodyUnbacked, got %v", err)
	}
