...
func TestDEXFillEvent_NotEmittedBeforeActivation(t *testing.T) {
	...
	before := h.state.stateDB.GetBalance(h.caller).ToBig().Int64()
	if _, err := h.runSwap(t, h.settlementCalldata(outputID, 250), false); err != nil {
		t.Fatalf("phase-B settle (pre-activation): %v", err)
	}
	// The credit DID happen (money path is unaffected by the log gate) ...
	after := h.state.stateDB.GetBalance(h.caller).ToBig().Int64()
	if after-before != 250 {
		t.Fatalf("pre-activation settle must still credit 250, got delta %d", after-before)
	}
