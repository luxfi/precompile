...
func TestNewQuantumVerifier(t *testing.T) {
	...
	if qv.CoronaKeys == nil {
		t.Error("Expected CoronaKeys map to be initialized")
	}
	if qv.MLDSAKeys == nil {
...
// TestRegisterCoronaKey tests Corona key registration
func TestRegisterCoronaKey(t *testing.T) {
	qv := NewQuantumVerifier()

	publicKey := make([]byte, 128) // Mock Corona public key
	for i := range publicKey {
		publicKey[i] = byte(i)
	}

	keyID, err := qv.RegisterCoronaKey(publicKey, 2, 5, CoronaParams{})
	if err != nil {
		t.Fatalf("RegisterCoronaKey failed: %v", err)
	}
...
// TestVerifyCorona tests Corona signature verification
func TestVerifyCorona(t *testing.T) {
	qv := NewQuantumVerifier()
	...
	keyID, _ := qv.RegisterCoronaKey(publicKey, 2, 5, CoronaParams{})
	...
	signature := &CoronaSignature{
		KeyID:      keyID,
		Signature:  []byte("test_corona_signature"),
		SignerMask: signerMask,
		Generation: 1,
	}

	message := []byte("test message")
	result, err := qv.VerifyCorona(keyID, message, signature)
	if err != nil {
		t.Fatalf("VerifyCorona failed: %v", err)
	}

	if result == nil {
		t.Fatal("Expected non-nil result")
	}
	if result.Algorithm != AlgCorona {
		t.Errorf("Expected Corona algorithm, got %v", result.Algorithm)
	}
	if result.GasUsed != GasCoronaVerify {
		t.Errorf("Expected gas %d, got %d", GasCoronaVerify, result.GasUsed)
	}
...
// TestVerifyCoronaKeyNotFound tests error for non-existent key
func TestVerifyCoronaKeyNotFound(t *testing.T) {
	qv := NewQuantumVerifier()

	nonExistent := [32]byte{0xFF}
	signature := &CoronaSignature{KeyID: nonExistent}

	_, err := qv.VerifyCorona(nonExistent, []byte("msg"), signature)
	if err != ErrKeyNotFound {
		t.Errorf("Expected ErrKeyNotFound, got %v", err)
	}
}

// TestVerifyCoronaGenerationMismatch tests generation validation
func TestVerifyCoronaGenerationMismatch(t *testing.T) {
	qv := NewQuantumVerifier()
	...
	_, err := qv.VerifyCorona(keyID, []byte("msg"), signature)
	if err != ErrInvalidSignature {
		t.Errorf("Expected ErrInvalidSignature, got %v", err)
	}
}

// TestVerifyCoronaThresholdNotMet tests threshold validation
func TestVerifyCoronaThresholdNotMet(t *testing.T) {
	qv := NewQuantumVerifier()

	publicKey := make([]byte, 128)
	keyID, _ := qv.RegisterCoronaKey(publicKey, 3, 5, CoronaParams{}) // Need 4 signers
...
func TestVerifyHybrid(t *testing.T) {
	...
	tests := []struct {
		...
		{"BLS+Corona both required", HybridBLSCorona, true},
		{"BLS+Corona either ok", HybridBLSCorona, false},
		{"ECDSA+ML-DSA both required", HybridECDSAMLDSA, true},
		{"Schnorr+Corona", HybridSchnorrCorona, false},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			} else if tt.scheme == HybridSchnorrCorona {
...
func TestDeriveAddress(t *testing.T) {
	...
	addr1 := qv.DeriveAddress(publicKey, AlgCorona)
	addr2 := qv.DeriveAddress(publicKey, AlgCorona)
	...
	addr3 := qv.DeriveAddress(differentKey, AlgCorona)
	if addr1 == addr3 {
...
func TestVerificationStatistics(t *testing.T) {
	...
	rtKeyID, _ := qv.RegisterCoronaKey(rtKey, 1, 3, CoronaParams{})
	...
	qv.VerifyCorona(rtKeyID, []byte("msg1"), sig)
	qv.VerifyCorona(rtKeyID, []byte("msg2"), sig)
...
func BenchmarkVerifyCorona(b *testing.B) {
	qv := NewQuantumVerifier()
	publicKey := make([]byte, 128)
	keyID, _ := qv.RegisterCoronaKey(publicKey, 2, 5, CoronaParams{})
	...
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		_, _ = qv.VerifyCorona(keyID, message, signature)
	}
}
