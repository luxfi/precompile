...
// - Corona operations (via NTT acceleration in luxcpp/lattice)
//
...
type GPUAccelerationAvailable struct {
	Corona bool   // Post-quantum via NTT
...
func (c *GPUOptimizedThresholdClient) GPUDeriveAddresses(keyIDs [][32]byte) (map[[32]byte]common.Address, error) {
	...
	for _, keyID := range keyIDs {
		...
		// Skip Corona - no EVM address derivation (post-quantum)
...
var gpuGasReductions = map[string]float64{
	...
	"Corona_Sign":  0.0, // Handled by separate corona precompile
