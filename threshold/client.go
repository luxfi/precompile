...
type ThresholdClient struct {
	...
	coronaConfigs map[[32]byte]*corona.Config
...
func NewThresholdClient() *ThresholdClient {
	...
	return &ThresholdClient{
		...
		coronaConfigs: make(map[[32]byte]*corona.Config),
	}
}
...
func (c *ThresholdClient) ExecuteKeygen(
	...
	case ProtocolCorona:
		return c.executeCoronaKeygen(ctx, threshold, participants, selfID)
	default:
...
func (c *ThresholdClient) executeCoronaKeygen(
	ctx context.Context,
	threshold int,
	participants []party.ID,
	selfID party.ID,
	...
	for _, id := range participants {
		...
		go func(id party.ID) {
			...
			h, err := protocol.NewMultiHandler(
				corona.Keygen(id, participants, threshold, c.pool),
				nil,
			)
			if err != nil {
				lastErr = err
			...
			configsMu.Lock()
			configs = append(configs, result.(*corona.Config))
			configsMu.Unlock()
		}(id)
	}
	...
	if lastErr != nil {
		return nil, fmt.Errorf("Corona keygen failed: %w", lastErr)
	}
	...
	keyID := sha256.Sum256(pubBytes)
	c.coronaConfigs[keyID] = ourConfig

	// Corona doesn't have EVM address derivation (post-quantum)
	return &KeygenResult{
		KeyID:     keyID,
		PublicKey: pubBytes,
		Address:   common.Address{},
...
func (c *ThresholdClient) ExecuteSigning(
	...
	case ProtocolCorona:
		return c.executeCoronaSign(ctx, keyID, messageHash, signers, selfID)
	default:
		return nil, ErrInvalidProtocol
	}
}
...
func (c *ThresholdClient) executeCoronaSign(
	ctx context.Context,
	keyID [32]byte,
	messageHash [32]byte,
	signers []party.ID,
	...
	config, ok := c.coronaConfigs[keyID]
	if !ok {
		return nil, ErrKeyNotFound
	}
	...
	if lastErr != nil {
		return nil, fmt.Errorf("Corona sign failed: %w", lastErr)
	}
...
func (c *ThresholdClient) ExecuteRefresh(
	...
	case ProtocolCorona:
		return c.executeCoronaRefresh(ctx, keyID, participants, selfID)
	default:
...
func (c *ThresholdClient) executeCoronaRefresh(
	ctx context.Context,
	keyID [32]byte,
	participants []party.ID,
	selfID party.ID,
	...
	for _, id := range participants {
		...
		go func(id party.ID) {
			...
			h, err := protocol.NewMultiHandler(
				corona.Refresh(config, participants, config.Threshold, c.pool),
				nil,
			)
			if err != nil {
				lastErr = err
			...
			configsMu.Lock()
			newConfigs = append(newConfigs, result.(*corona.Config))
			configsMu.Unlock()
		}(id)
	}
	...
	if lastErr != nil {
		return fmt.Errorf("Corona refresh failed: %w", lastErr)
	}

	for _, cfg := range newConfigs {
		if cfg.ID == selfID {
			c.coronaConfigs[keyID] = cfg
			break
		}
	}
...
func (c *ThresholdClient) ExecuteReshare(
	...
	case ProtocolCorona:
		return c.executeCoronaReshare(ctx, keyID, newParticipants, newThreshold, selfID)
	default:
...
func (c *ThresholdClient) executeCoronaReshare(
	ctx context.Context,
	keyID [32]byte,
	newParticipants []party.ID,
	newThreshold int,
	...
	for _, id := range newParticipants {
		...
		go func(id party.ID) {
			...
			h, err := protocol.NewMultiHandler(
				corona.Refresh(config, newParticipants, newThreshold, c.pool),
				nil,
			)
			if err != nil {
				lastErr = err
	...
	if lastErr != nil {
		return [32]byte{}, fmt.Errorf("Corona reshare failed: %w", lastErr)
	}
	...
	delete(c.coronaConfigs, keyID)
	c.coronaConfigs[newKeyID] = ourConfig
...
func (c *ThresholdClient) VerifySignature(
	...
	case ProtocolCorona:
		...
		return corona.VerifySignature(config.PublicKey, messageHash[:], signature), nil
